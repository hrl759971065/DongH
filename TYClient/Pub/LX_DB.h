...
enum TParamDirect {
	spUnknown, spInput, spOutput, spInputOutput, spReturnValue
}; // ---存储过程的参数类型
...
public:
	...
	virtual void FieldAsName_B6(int nRows, WideString Name,
		WideString FieldVal) = 0;
	virtual void FieldAsName(WideString Name, Variant FieldVal) = 0;
	virtual void FieldAsName_B6(WideString Name, WideString FieldVal) = 0;
	////B6以上通用
	...
	virtual void GetTableNames(TStrings* TabList, bool AllTables) = 0;
	// 2010及以上使用(UniCode)
	virtual void GetTableNames(WideString &TabList, bool AllTables =
		false) = 0; // BCB通用
...
typedef TLX_DB* (__stdcall *pFnCreateLX_DB)(void);

extern "C" __declspec(dllexport) TLX_DB* __stdcall CreateLX_DB(void);
